...
// 分水岭： 424 engine.start() 或 443 connector.start(); 也可以不在Bootstrap那里断点 从头开始，从这断点也行：engine或connector
    ...
    // 进去 start connector->protocolHandler(endpoint+processor)->adapter    启动Connector连接器
                        connector.start();// TODO: 可以加断点
                    }
                } catch (Exception e) {
                    log.error(sm.getString(
                            "standardService.connector.startFailed",
...
// 这里 是一个分水岭: 538:engine.init() + 558:connector.init()
        super.initInternal();

        if (engine != null) {
    // 进去 init 再出来 engine:StandardEngine->ContainerBase  Servlet容器引擎engine组件初始化
            engine.init();// TODO: 可以加断点
        }
