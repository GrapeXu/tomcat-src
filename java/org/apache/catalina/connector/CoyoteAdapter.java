...
public class CoyoteAdapter implements Adapter {
    ...
    // CoyoteAdapter需要将原生Request和Response进行进一步封装为HttpServletRequest和HttpServletResponse
    @Override
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
            throws Exception {
// 341 postParseRequest 348 invoke 可加断点直接跳过去
        // 下面的Request和Response是HttpServletRequest和HttpServletResponse(ctrl看一下还真是impl了)，先看一下是否为空，为空就需要转换原生
        Request request = (Request) req.getNote(ADAPTER_NOTES);// 适配
        Response response = (Response) res.getNote(ADAPTER_NOTES);// 适配
        ...
        try {
            ...
            // request parameters
    // 进去 出来 在这个方法中会根据请求信息找到能够处理当前请求的HOST->CONTEXT->WRAPPER(SERVLET) 通过Mapper 找到对应servlet 适配好req resp
            postParseSuccess = postParseRequest(req, request, res, response);// TODO:可加断点
            if (postParseSuccess) {
                //check valves if we support async
                request.setAsyncSupported(
                        connector.getService().getContainer().getPipeline().isAsyncSupported());
                // Calling the container
    // 进去 3?4次invoke里 上面匹配出能够处理当前请求的容器知乎，就开始一级一级深入，最后取出servlet执行：Engine->Host->Context->FilterChain
                connector.getService().getContainer().getPipeline().getFirst().invoke(
                        request, response);// TODO:可加断点
            }
            if (request.isAsync()) {
                async = true;
                ReadListener readListener = req.getReadListener();
...
// 698 connector.getService().getMapper().map 可以直接跳
        ...
        while (mapRequired) {
            // This will map the the latest version by default 进的是map一共四个方法里的最后一个...看一个request.getMappingData()
            connector.getService().getMapper().map(serverName, decodedURI,
                    version, request.getMappingData());// TODO:设置断点 回来后，这里通过表达式看request.getMappingData()
