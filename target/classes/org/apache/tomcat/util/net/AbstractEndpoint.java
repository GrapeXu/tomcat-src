...
// 1075 ctrl->SocketProcessorBase.run():doRun() 加断点 直接跳
        try {
            if (socketWrapper == null) {
                return false;
            }

            // 从 缓存池 中 获取一个 SocketProcessor处理线程 sc是处理线程
            SocketProcessorBase<S> sc = processorCache.pop();// TODO:从这里ctrl去 SocketProcessorBase.run():doRun() 加断点
            if (sc == null) {
                // 缓存池中没有了，就创建 (并传入socket|socketWrapper)
            ...
            if (dispatch && executor != null) {
                ...
                executor.execute(sc);// executor是一个线程池，execute是通过线程池去执行某个线程
            } else {
                sc.run();
            }
...
// 1126 bind() 可以直接跳过去
        if (bindOnInit) {
    // 进去 init connector:(Connector:Adapter)->(protocolHandler:AbstractHttp11Protocol->AbstractProtocol->AbstractJsseEndpoint->AbstractEndpoint->NioEndpoint)
            bind();// TODO: 可以加断点
...
// 1204 startInternal()
    ...
    // 进去 start Connector connector->protocolHandler(endpoint+processor)->adapter
        startInternal();// TODO: 可以加断点
    }
...
// 1214 createAcceptor
    ...
    // 进去 start 创建Acceptor connector->protocolHandler(endpoint+processor)->adapter
            acceptors[i] = createAcceptor();// TODO: 可以加断点
