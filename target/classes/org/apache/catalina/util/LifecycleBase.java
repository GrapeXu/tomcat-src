...
/**
 * 抽象类，即便不实现特定方法，也可以对其流程规范等进行管理，具体实现交由子类
...
public abstract class LifecycleBase implements Lifecycle {
    ...
    // 进去 start engine->host->context->wrapper
            listener.lifecycleEvent(event);// TODO: 可以加断点
        }
    }

    /**
     * 每个组件初始化都会来到该类该方法，因为这些组件都实现了Lifecycle
    ...
    // 进去 init 的关键之处 bootstrap|catalina|daemon->server|StandardServer->(service|StandardService:engine、connector)
            initInternal();// TODO:可以加断点 抽象方法，交由子类完成。模板设计
...
// 191 startInternal() 可以跳过去
    ...
    // 进去 start bootstrap|catalina|daemon->server|StandardServer->(service|StandardService:(engine:engine->host->context->wrapper)、connector:connector->protocolHandler(endpoint+processor)->adapter )
            startInternal();// TODO: 可以加断点 模板方法 子类实现
    ...
    protected synchronized void setState(LifecycleState state) throws LifecycleException {
        // Host这里是个包装
// 进去 start engine->host->context->wrapper
        setStateInternal(state, null, true);// TODO: 可以加断点
    }
...
// 435 fireLifecycleEvent 可以跳过去
        ...
        if (lifecycleEvent != null) {
            // Host 触发生命周期事件
    // 进去 start engine->host->context->wrapper
            fireLifecycleEvent(lifecycleEvent, data);// TODO: 可以加断点
        }
    }
