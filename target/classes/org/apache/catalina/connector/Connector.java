...
// 1082 protocolHandler.init()可以加断点
        super.initInternal();

        // Initialize adapter
// 初始化adapter，并和protocolHandler关联起来
        adapter = new CoyoteAdapter(this);//Adapter
// protocolHandler 和 Adapter 绑定
        protocolHandler.setAdapter(adapter);//protocolHandler=endpoint + processor
        ...
        try {
            // protocolHandler组件的初始化，主要是其内部的Endpoint的初始化
    // 进去 init connector:(Connector:Adapter)->(protocolHandler:AbstractHttp11Protocol->AbstractProtocol->AbastractJsseEndpoint->AbstractEndpoint->NioEndpoint)
            protocolHandler.init();// TODO: 可以加断点
        } catch (Exception e) {
...
// 1108 protocolHandler.start()
    ...
    // 进去 start connector->protocolHandler(endpoint+processor)->adapter
            protocolHandler.start();// TODO: 可以加断点
        } catch (Exception e) {
            throw new LifecycleException(
