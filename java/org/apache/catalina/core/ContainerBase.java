...
// 721 addChildInternal
        if (Globals.IS_SECURITY_ENABLED) {
            PrivilegedAction<Void> dp =
                new PrivilegedAddChild(child);
            AccessController.doPrivileged(dp);
        } else {
    // 进去 start engine->host->context->wrapper
            addChildInternal(child);// TODO: 可以加断点
        }
    }
...
// 747 child.start()
     ...
     */
    @Override
    protected void initInternal() throws LifecycleException {// 到这就结束了,连跳3次会跳会service处的分水岭:engine?connector
...
// 938：StartChild 跳到call里去(需要在call中提前设置断点)   967:setState
    ...
    // 第一次：进去|跳 start resume跳到下一个断点 第二次：过去，在下面setState处进去
        // TODO:需要设置                             StartChild的call()处 断点
        for (Container child : children) {
            // 在这 resume program 跳到下一个断点就是StartChild里的child.start()
            // 线程池 提交线程。Context应该也是这线程池下一层
            results.add(startStopExecutor.submit(new StartChild(child)));// TODO: ctrl-> StartChild.call()处加断点，跳到call里去
        }
        ...
        // (Host实例化时进) 设置容器生命周期状态(start) 每个Host的实例化是靠生命周期事件 触发事件驱动 来走的 fireLifecycleEvent
    // 进去 start engine->host->context->wrapper
        setState(LifecycleState.STARTING);// TODO: 加断点
    ...
    private static class StartChild implements Callable<Void> {
        ...
        public Void call() throws LifecycleException {
            // StartChild的核心逻辑
    // 进去 start engine->host->context->wrapper
            child.start();// TODO:此处需要设置断点 为了start
            return null;
        }
    }
