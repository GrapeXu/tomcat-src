...
public class Http11Processor extends AbstractProcessor {
    ...
    // 应用层处理器Http11Processor 对请求进行 解析处理。解析http报文，封装元素的Request、Response对象
    @Override
    public SocketState service(SocketWrapperBase<?> socketWrapper)
        throws IOException {
// 620 getAdapter().service 可设置断点直接跳过去
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null &&
            ...
            // ============END,主要在解析封装元素Request对象以及初步的Response对象，后续要传入Adapter适配
            // Process the request in the adapter
            if (getErrorState().isIoAllowed()) {
                try {
                    rp.setStage(org.apache.coyote.Constants.STAGE_SERVICE);
    // 进去 service() Processor正式把接力棒交到了Adapter手中，由Adapter继续处理Request和Response；()
                    getAdapter().service(request, response);// TODO: 可设置断点
