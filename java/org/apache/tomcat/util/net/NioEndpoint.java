...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    // 到这就结束了
        ...
        // 绑定端口，但尚未使用accept获取客户端连接 addr就是端口
...
// 288 startAcceptorThreads
        if (!running) {
            ...
            // Acceptor是用来接收请求的线程
    // 进去 start connector->protocolHandler(endpoint+processor)->adapter
            startAcceptorThreads();// TODO: 可以加断点
        }
    }
    ...
    protected AbstractEndpoint.Acceptor createAcceptor() {
        // start connector->protocolHandler(endpoint+processor)->adapter
        return new Acceptor();// ctrl 去看看Acceptor线程，到此结束
    }
...
// socket 获取socket连接 到这就差不多结束了 connector->protocolHandler(endpoint+processor)->adapter
    ...
    /**
     * 检查Selector多路复用器中是否有可以被处理的Socket
     * Poller class.
     */
    public class Poller implements Runnable {
        ...
        public void run() {
            ...
            while (true) {// 循环检查selector里有没有需要被处理的channel
                ...
                try {
                    ...
                    if (!close) {
                        hasEvents = events();
                        // keyCount selector中有几个需要被处理的 channel|socket
                        // 如果返回值>0，说明selector中有事件就绪|有请求需要处理了
                        if (wakeupCounter.getAndSet(-1) > 0) {
                            // if we are here, means we have other stuff to do
                            // do a non blocking select keyCount
                ...
                if ( close ) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            // 读数据
                            if (sk.isReadable()) {
                                // 调用processSocket具体处理当前Socket attachment≈socket
    //进去 servlet NioEndpoint.Poller.processKey().processSocket()  这里是分析入口
                                if (!processSocket(attachment, SocketEvent.OPEN_READ, true)) {// TODO: 可以加断点
                                    closeSocket = true;
                                }
                            }
                            // 写数据
    ...
    protected class SocketProcessor extends SocketProcessorBase<NioChannel> {
        ...
        /**
         * 重写父类 SocketProcessorBase中的doRun方法
         */
        @Override
        protected void doRun() {
// 1668 getHandler().process  可以直接跳
            // 从wrapper包装对象中拿出Channel对象(可理解为socket套接字) 准备去处理
    ...
    // 进去 process(两次) 方法会获取一个Processor进行(应用层协议)处理
                        state = getHandler().process(socketWrapper, event);// TODO:可设置断点
                    }
