...
// 590 endpoint.init() 可以直接跳过去
        ...
        // 通信端点的初始化(绑定端口)
    // 进去 init connector:(Connector:Adapter)->(protocolHandler:AbastractHttp11Protocol->AbstractProtocol->AbastractJsseEndpoint->AbstractEndpoint->NioEndpoint)
        endpoint.init();// TODO: 可以加断点
    }
...
// 835 state = processor.process(wrapper, status) 可以设断点直接跳
            // 日志输出
            if (getLog().isDebugEnabled()) {
            ...
            try {
                ...
                // 上面如果没有找到的话从recycleProcessors中获取，也就是已经处理过连接但是没有被销毁的Processor，避免频繁的创建和销毁
                if (processor == null) {
                ...
                // 后面我们就需要调用Http11Processor对请求进行简单的内容解析之后，则调用Adapter(连接适配器)的方法，将请求转发给
                if (processor == null) {
                ...
                do {
    // 进去 调用应用层处理器处理，比如Http11Processor进行请求内容的解析
                    state = processor.process(wrapper, status);// TODO:可以设置断点
