...
public final class Bootstrap {
    ...
    //为调用Catalina对象的setParentClassLoader方法做准备:方法名+参数
    ...
    private void load(String[] arguments) throws Exception {
        ...
        String methodName = "load";//要调用的catalina的方法名
        Object param[];
        Class<?> paramTypes[];
        ...
        //Catalina.load()
    // 进去 init bootstrap|catalina|daemon->server|StandardServer->service|StandardService
...
// 355 method.invoke可以直接跳过去
    ...
    // 进去 start bootstrap|catalina|daemon->server|StandardServer->service|StandardService
        method.invoke(catalinaDaemon, (Object [])null);// TODO: Catalina.start() 可以加断点
    }
...
// 492 load 495 start 可直接跳过去
        synchronized (daemonLock) {
            if (daemon == null) {
    // 实例化一个档期Bootstrap引导类对象
    ...
    // 把当前Bootstrap引导类对象赋值给一个变量daemon
        ...
        try {
            ...
            if (args.length > 0) {
                command = args[args.length - 1];//这里涉及到了命令行参数的使用
            }
            ...
            if (command.equals("startd")) {
                ...
                daemon.stop();
            } else if (command.equals("start")) {// 启动tomcat时传入的是start命令参数，所以走此分支
                daemon.setAwait(true);
                // 启动时候非常重要的一步 load 加载初始化
    // 进去 init bootstrap|catalina|daemon->server|StandardServer->service|StandardService
                daemon.load(args);// TODO:init入口断点
                // 启动时非常重要的一步 start 启动
    // 进去 start bootstrap|catalina|daemon->server|StandardServer->service|StandardService
                daemon.start();// TODO:start入口断点
